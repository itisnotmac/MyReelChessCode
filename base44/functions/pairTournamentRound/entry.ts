...
Deno.serve(async (req) => {
  try {
    ...
    // Survivors = non-eliminated, ordered by bracket seed (1 = top seed)
    ...
    if (survivors.length <= 1) {
      ...
      return Response.json({ ok: true, completed: true, champion: survivors[0]?.user_id });
    }

    const maxRound = games.reduce((m, g) => Math.max(m, g.tournament_round || 0), 0);

    if (maxRound === 0) {
      // Round 1 — pair all survivors (none eliminated yet) sequentially; odd seed gets a bye
      const matches = Math.floor(survivors.length / 2);
      await createRound(base44, tournament_id, 1, survivors);
      await base44.asServiceRole.entities.Tournament.update(tournament_id, { current_round: 1 });
      return Response.json({ ok: true, round: 1, matches });
    }

    // Wait for the current round to finish before advancing
    const currentRoundGames = games.filter(g => (g.tournament_round || 0) === maxRound);
    ...
    // Guard against duplicate advancement (two clients settling near-simultaneously)
    const existingNext = games.filter(g => (g.tournament_round || 0) === nextRound);
    if (existingNext.length > 0) {
      return Response.json({ ok: true, round: nextRound, message: 'Next round already created' });
    }

    const matches = Math.floor(survivors.length / 2);
    await createRound(base44, tournament_id, nextRound, survivors);
    await base44.asServiceRole.entities.Tournament.update(tournament_id, { current_round: nextRound });
    return Response.json({ ok: true, round: nextRound, matches });
  } catch (error) {
...
// Pairs survivors two-by-two (sequential seeding). An odd final survivor gets a
// bye (no game created) and automatically advances to the next round.
async function createRound(base44, tournament_id, round, survivors) {
  for (let i = 0; i + 1 < survivors.length; i += 2) {
    const host = survivors[i];
    const guest = survivors[i + 1];
